...
function applyDiff(
  ...
  // Reference equality guards: skip entire sections if nothing changed.
  // This avoids O(n²) Automerge proxy accesses for position-only updates.
  if (prev.entities !== next.entities) {
    ...
    for (const nextEntity of next.entities) {
      ...
      if (prevEntity === nextEntity) continue; // same reference = unchanged
