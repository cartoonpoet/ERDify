...
- **모달 프레임 전체**: 흰 배경 (기존 Modal 컴포넌트 그대로 유지, 다크 NO)
- **Dialect 탭**: 언더라인 스타일 상단 탭 (MySQL | PostgreSQL | MariaDB | MSSQL) — 라이트 배경
- **코드 에디터 영역만** 다크 배경 (`#0d1821`) + 왼쪽 라인 번호 컬럼
- **그 외 영역** (이름 필드, 힌트, 버튼): 모두 라이트 테마 유지
