import type { DiagramDocument, DiagramEntity, DiagramIndex, DiagramRelationship } from "../types/index.js";
...
/** PK 누락 / nullable PK */
function findPrimaryKeyIssues(entities: DiagramEntity[]): SchemaFinding[] {
...
/** 인덱스 없는 FK 컬럼 */
function findFksWithoutIndex(entities: DiagramEntity[], relationships: DiagramRelationship[], indexes: DiagramIndex[]): SchemaFinding[] {
  const findings: SchemaFinding[] = [];
...
/** 같은 컬럼명, 다른 타입 (FK 타입 불일치 후보) */
function findTypeMismatches(entities: DiagramEntity[]): SchemaFinding[] {
  const findings: SchemaFinding[] = [];
...
/** 네이밍 케이스 혼용 (snake vs camel) */
function findNamingInconsistency(entities: DiagramEntity[]): SchemaFinding[] {
...
/** 반복 컬럼군 (addr1, addr2 / phone_1, phone_2 → 정규화 후보) */
function findRepeatingGroups(entities: DiagramEntity[]): SchemaFinding[] {
  const findings: SchemaFinding[] = [];
  for (const entity of entities) {
 ...
 *
 * findings 배열 순서는 MAX_FINDINGS 절단 시 어떤 finding이 살아남는지를 결정하므로
 * 블록 순서(PK → FK 인덱스 → 타입 → 네이밍 → 반복군)를 바꾸면 안 된다.
 */
export function analyzeSchema(doc: DiagramDocument): SchemaFinding[] {
  ...
  const findings = [
    ...findPrimaryKeyIssues(entities),
    ...findFksWithoutIndex(entities, relationships, indexes),
    ...findTypeMismatches(entities),
    ...findNamingInconsistency(entities),
    ...findRepeatingGroups(entities),
  ];
